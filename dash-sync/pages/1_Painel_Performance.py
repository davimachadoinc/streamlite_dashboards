"""
pages/1_Painel_Performance.py
DASH SYNC — Painel de Performance (semana a semana, últimas 10 colunas).
"""
import streamlit as st
import plotly.graph_objects as go

st.set_page_config(page_title="Performance | Dash Sync", page_icon="📈", layout="wide")
...
from utils.sheets import load_performance
...
_BAR_UNITS = {"num", "moeda", ""}   # exibe como barra; outros → linha
...
def _chart_layout(title: str = "") -> dict:
    return dict(
        ...
        title=dict(text=title, font=dict(size=12, color="#ffffff"), x=0, pad=dict(l=4)),
        margin=dict(l=0, r=0, t=32 if title else 8, b=0),
        xaxis=dict(
...
def _indicator_card(ind: dict) -> None:
    ...
    if meta is not None:
        fig.add_hline(
            ...
            line_dash="dash",
            line_color="#4c4c4c",
            annotation_text=f"Meta: {ind['meta_raw']}",
...
# ── Carga ───────────────────────────────────────────────────────────────
st.markdown("<h1>Painel <span>Performance</span></h1>", unsafe_allow_html=True)

with st.spinner("Carregando dados da planilha..."):
    indicators = load_performance()
...
with col_f:
    area_filter = st.selectbox("Área", ["Todas"] + areas, key="perf_area")
    ...
    for pair in pairs:
        ...
        for col, ind in zip(cols, pair):
            with col:
                with st.container():
                    st.markdown(
                        f"<div style='background:#121212;border:1px solid #292929;border-radius:12px;padding:16px 16px 4px 16px;margin-bottom:12px;'>",
                        unsafe_allow_html=True,
                    )
                    _indicator_card(ind)
