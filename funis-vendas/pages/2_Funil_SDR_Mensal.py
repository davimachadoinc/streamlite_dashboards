"""
pages/2_Funil_SDR_Mensal.py
Funil de SDR — visão por mês/evento (sem coorte).
Mesmo desenho visual da Página 1, mas o filtro de mês conta transições que
aconteceram naquele mês (não o mês de Entrada do lead) — populações diferentes
podem compor cada barra. Ver spec completa em Obsidian:
...
st.set_page_config(
    page_title="Funil de SDR (Mensal) | InChurch",
    page_icon="📅",
    layout="wide",
)
...
st.session_state["_page_key"] = "funil_sdr_mensal"
...
st.markdown("<h1>Funil de SDR <span>— Mensal</span></h1>", unsafe_allow_html=True)
st.caption(
    "Filtro por mês do EVENTO (não coorte) — cada barra conta transições que aconteceram no mês "
    "selecionado. Um lead pode contribuir pra 'Entrada' num mês e pra 'Reunião Agendada' em outro; "
    "as barras podem representar populações de leads diferentes entre si. Para leitura de coorte "
    "(percurso completo por mês de entrada), use a página 'Funil de SDR — Coorte'."
)
...
with st.sidebar:
    ...
    meses = (
        df[["_mes_evento_fmt", "_mes_evento_ord"]]
        .dropna()
        .drop_duplicates()
        .sort_values("_mes_evento_ord")["_mes_evento_fmt"]
        .tolist()
    )
    sel_meses = st.multiselect("Mês do Evento", meses, placeholder="Todos")
...
dfv = df.copy()
if sel_meses:
    dfv = dfv[dfv["_mes_evento_fmt"].isin(sel_meses)]
if sel_sdr:
...
if dfv.empty:
    st.warning("Nenhuma transição encontrada para os filtros selecionados.", icon="⚠️")
    st.stop()
...
c1.metric("Leads com Transição no Período", fmt_int(n_leads))
...
fig = go.Figure(
    go.Funnel(
        ...
        hovertemplate=(
            "<b>%{y}</b><br>Transições no período: %{x}<br>%{percentInitial} da Entrada"
...
st.caption(
    "**Em Aberto**: leads parados nessa etapa, sem avançar e sem desqualificação (dentro do "
    "período filtrado). **Desqualificado**: leads desqualificados saindo dessa etapa. Os dois não "
    "são mutuamente exclusivos entre si nem com 'Avançou' — não somam 100% do trapézio."
)
            ...
            <div style="display:flex; align-items:center; justify-content:center;
              ...
              <div>
                ...
                <div style="color:{PALETTE_GREEN}; font-size:0.85rem; font-weight:600;">
                  {fmt_int(row['alcancou'])} no período · {fmt_pct(row['pct_entrada'])} da Entrada
                </div>
              </div>
            </div>
            """,
